...
real scale = 1.0; // scale each slice for normalization
int distance_weighting = 1; // Flag to control the distance weighting of cooccurrence counts
char *vocab_file, *file_head, *cooccur_file;
...
int write_chunk(CREC *cr, long long length, FILE *fout) {
    ...
    for (a = 1; a < length; a++) {
        if (cr[a].word1 == old.word1 && cr[a].word2 == old.word2 && cr[a].word3 == old.word3) {
            old.val += cr[a].val;
            continue;
        }
...
int compare_crec(const void *a, const void *b) {
    ...
    else if ( (d = ((CREC *) a)->word2 - ((CREC *) b)->word2) != 0 ) return d;
    else return (((CREC *) a)->word3 - ((CREC *) b)->word3); // DIFF
...
int compare_crecid(CRECID a, CRECID b) {
    ...
    else if ( (d = a.word2 - b.word2) != 0) return d;
    else return a.word3 - b.word3; // DIFF
}
...
int merge_write(CRECID new, CRECID *old, FILE *fout) {
    ...
    scale_old.word1 = old->word1;
    scale_old.word2 = old->word2;
    scale_old.word3 = old->word3;
    scale_old.val = scale * old->val;
    fwrite(&scale_old, sizeof(CREC), 1, fout);
    //fwrite(old, sizeof(CREC), 1, fout); // DIFF: fwrite(old, sizeof(CREC), 1, fout)
    *old = new;
...
int merge_files(int num) {
    int i, size;
    long long counter = 0;
    CRECID *pq, new, old, scale_old;
    char filename[200];
    FILE **fid, *fout;
    fid = malloc(sizeof(FILE) * num);
    pq = malloc(sizeof(CRECID) * num);
    //fout = stdout;
    fout = fopen(cooccur_file, "ab");
...
int get_cooccurrence() {
    ...
    //sprintf(stderr, "vocab file: %s", vocab_file);
    ...
    for (x = 1; x <= vocab_size; x++) {
        ...
        for (y = 1; y <= (lookup[x] - lookup[x-1]); y++) {
            if ((r = bigram_table[lookup[x-1] - 2 + y]) != 0) {
                ...
                //fwrite(&cond, sizeof(int), 1, fid);
                //fwrite(&r, sizeof(real), 1, fid);
                new.word1 = x;
                new.word2 = y;
                new.word3 = cond;
                new.val = r;
                fwrite(&new, sizeof(CREC), 1, fid);
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    cooccur_file = malloc(sizeof(char) * MAX_STRING_LENGTH);
    ...
    if ((i = find_arg((char *)"-cooccur-file", argc, argv)) > 0) strcpy(cooccur_file, argv[i + 1]);
    else strcpy(cooccur_file, (char *)"cooccur.bin");   
    ...
    if ((i = find_arg((char *)"-cond", argc, argv)) > 0) cond = atoi(argv[i + 1]); // DIFF
    if ((i = find_arg((char *)"-scale", argc, argv)) > 0) scale = atof(argv[i + 1]);
    if ((i = find_arg((char *)"-distance-weighting", argc, argv)) > 0)  distance_weighting = atoi(argv[i + 1]);
    else distance_weighting = 1;
