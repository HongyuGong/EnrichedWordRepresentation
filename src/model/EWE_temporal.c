...
real eta = 0.04; // 0.02, Initial learning rate: 0.05
...
real year_reg = 1.5; // 1.0, weight on time vector l2 regularizer
...
void *ewe_thread(void *vid) {
    long long a, b ,l1, l2, l3, l3_prev, l3_next, l5, l6, l7, l8;
    ...
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        // previous year of time 1 in T
        l3_prev = (cr.word3 - 1LL - 1LL) * vector_size;
        l3_next = cr.word3 * vector_size;
        ...
        for (b = 0; b < vector_size; b++) {
            ...
            if (l3_prev >= 0) {
            	Ttemp1 += eta * year_reg * (T[l3 + b] - T[l3_prev + b]);
            }
            if (l3_next + vector_size < condition_size * vector_size) {
                Ttemp1 += eta * year_reg * (T[l3 + b] - T[l3_next + b]);
            }
...
int train_ewe() {
    ...
    for (b = 0; b < num_iter; b++) {
        ...
        //fprintf(stderr, "line 379 is good"); // DEBUG
